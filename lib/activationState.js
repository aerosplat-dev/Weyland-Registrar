...
/**
 * Names of every currently-active collection that contains this item --
 * shown as a "from <Collection>" tag so a user can see WHY an unpinned item
 * is active, and that deactivating it individually has no effect (see
 * resolveItemActive's own doc above) -- the collection itself must be
 * deactivated, or the item pinned, to turn it off.
 * @param {string} itemKey
 * @param {Object.<string, {active: boolean, memberKeys: string[], name: string}>} collections
 * @returns {string[]}
 */
export function resolveActiveCollectionNames(itemKey, collections) {
    const names = [];
    for (const collectionId in collections) {
        const collection = collections[collectionId];
        if (collection.active && collection.memberKeys.includes(itemKey)) {
