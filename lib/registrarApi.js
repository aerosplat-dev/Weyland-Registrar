 ...
 * CORS-blocked. Same three services, in the same order, as the proven
 * pattern already shipping in the bundled Weyland-WeyPhone extension's dorm
 * floormap (maps/weyland_dorms.html's CORS_PROXIES) -- switched to from this
 * extension's original approach (SillyTavern's own /proxy/ passthrough,
 * gated on enableCorsProxy: true in config.yaml) after real users reported
 * connection failures reaching the Registrar. Unlike the ST-proxy route,
 * these run entirely client-side against independent third-party services,
 * so a user's own server config/hosting setup can't be the point of failure.
 *   - api.codetabs.com   -> fastest in WeyPhone's own testing, no rate caveats hit so far.
 *   - api.allorigins.win -> slower, occasionally returns wrapped responses.
 *   - corsproxy.io       -> can 413 on large payloads, kept as a last resort.
 */
