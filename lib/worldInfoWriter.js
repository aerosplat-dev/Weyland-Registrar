...
import { ROSTER_UID, LOCATION_LIST_UID, MARKER_UID } from './uidScheme.js';
import { ensureBookOwnership, buildMarkerEntry } from './bookOwnership.js';
...
/**
 * Required sequence: back up foreign pre-existing content (if any) -> write
 * -> refresh book list -> activate. Never reorder the write/refresh/activate
 * part -- skipping updateWorldInfoList before activating silently toasts
 * "No world found" instead of erroring. The marker entry (bookOwnership.js)
 * is injected here, once, so callers (syncCharacterBook/syncLocationBook)
 * don't each need to know about it.
 ...
 * @returns {Promise<string|null>} the backup book's name if pre-existing unmarked content was found, else null
 */
async function writeAndActivate(stContext, bookName, bookData) {
    const backupBookName = await ensureBookOwnership(stContext, bookName);
    const entries = { ...bookData.entries, [MARKER_UID]: buildMarkerEntry() };
    await stContext.saveWorldInfo(bookName, { ...bookData, entries }, true);
    await stContext.updateWorldInfoList();
    await stContext.executeSlashCommandsWithOptions(`/world state=on silent=true "${bookName}"`);
    return backupBookName;
}
 ...
 * @returns {Promise<string|null>} see writeAndActivate
 */
export async function syncCharacterBook(stContext, callFunction, settings, allCharacterRecordsByKey) {
    ...
    entries[ROSTER_UID] = {
        ...
        constant: true,
        // This entry's content lists every active character's name. Without
        // this, ST's recursive WI scan (enabled in this deployment) feeds
        // that listing back into the buffer, and since every character's own
        // key is just their name, the whole roster cascade-activates on
        // every message regardless of what's actually said -- confirmed live
        // (a single-name test message activated 47 entries with this unset,
        // 21 -- all legitimate -- with it set).
    ...
    return writeAndActivate(stContext, CHARACTER_BOOK_NAME, { entries });
}
...
export async function syncLocationBook(stContext, callFunction, settings, allLocationRecordsByKey) {
    ...
    entries[LOCATION_LIST_UID] = {
        ...
        constant: true,
        // Same recursion-cascade hazard as ROSTER_UID above -- this entry's
        // content lists every active location's name.
        preventRecursion: true,
        position: 1,
        disable: false,
        order: 8000,
    ...
    return writeAndActivate(stContext, LOCATION_BOOK_NAME, { entries });
}
