...
async function buildModalElement() {
    ...
    portal.querySelectorAll('.wreg-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            updateStatsBar(tab.dataset.type);
        });
    });
 ...
 * summary + the Rebuild Lorebook button, both scoped to whichever of the two
 * WI-book-backed tabs is current -- hidden entirely on the Collections/Local
 * Sets tabs, which have no book of their own to rebuild or count. The active
 * item count is synchronous (derived from already-exposed handlers); the
 * live lorebook entry count needs an actual WI read, so it's rendered once
 * the count resolves, not blocking the rest of the bar from appearing.
 * @param {string} type
 */
async function updateStatsBar(type) {
