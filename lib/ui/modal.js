...
export function openModal(state) {
    ...
    ensureModalElement().then((window_) => {
        ...
        updateRebuildButton(currentTabType());
...
async function buildModalElement() {
    ...
    portal.querySelectorAll('.wreg-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            updateRebuildButton(tab.dataset.type);
        });
    });
    ...
    portal.querySelector('#wreg-rebuild-lorebook').addEventListener('click', async (event) => {
        ...
        // moment that distinguishes "Apply" from a plain "Rebuild" for the
        // in-flight label; the resting label afterward is set by
        // updateRebuildButton below from whatever isDirty(bookType) actually
        // is post-operation (false on success, unchanged on failure -- see
        // rebuildLorebook's own doc in index.js).
...
/**
 * Shows/updates the titlebar's Rebuild/Apply button, scoped to whichever of
 * the two WI-book-backed tabs is current -- hidden entirely on the
 * Collections/Local Sets tabs, which have no book of their own.
 * @param {string} type
 */
function updateRebuildButton(type) {
    const button = portalElement.querySelector('#wreg-rebuild-lorebook');
    ...
    button.textContent = dirty ? 'Apply' : 'Rebuild';
    button.classList.toggle('wreg-btn-glow', dirty);
}
...
function renderCurrentTab(type) {
    ...
    renderItemList(container, filtered, {
        ...
        onActivate: (itemKey) => { currentState.onActivate(itemKey); renderCurrentTab(currentTabType()); updateRebuildButton(currentTabType()); },
        onDeactivate: (itemKey) => { currentState.onDeactivate(itemKey); renderCurrentTab(currentTabType()); updateRebuildButton(currentTabType()); },
...
function openDetail(itemKey) {
    ...
    renderDetailPane(container, detail, {
        ...
        onActivate: (key) => { currentState.onActivate(key); openDetail(key); renderCurrentTab(currentTabType()); updateRebuildButton(currentTabType()); },
        onDeactivate: (key) => { currentState.onDeactivate(key); openDetail(key); renderCurrentTab(currentTabType()); updateRebuildButton(currentTabType()); },
