 ...
 * @property {string[]} [activeCollectionNames] - names of every currently-active collection this item belongs to
...
export function renderDetailPane(container, detail, handlers) {
    ...
    if (detail.activeCollectionNames?.length) {
        const badges = document.createElement('div');
        badges.className = 'wreg-detail-collection-badges';
        for (const name of detail.activeCollectionNames) {
            const badge = document.createElement('span');
            badge.className = 'wreg-collection-badge';
            badge.textContent = name;
            badge.title = `Active because of the "${name}" collection`;
    ...
    if (detail.kind === 'character' || detail.kind === 'location') {
        // Independent of the toggle above -- pinning has to work even when
        // this item is already active via a collection (the toggle above
        // has no "activate" affordance in that case, since it's already on).
        // Reuses the same onActivate/onDeactivate handlers: pinning is
        // exactly "force itemStates[itemKey] to 'active'", unpinning is
        // exactly "clear it" -- just keyed off `forced` instead of `isActive`.
        const pinButton = document.createElement('button');
        pinButton.type = 'button';
        pinButton.className = 'wreg-pin-btn wreg-pin-btn-detail' + (detail.forced === 'active' ? ' wreg-pin-btn-active' : '');
        pinButton.textContent = '📌';
        pinButton.title = detail.forced === 'active'
        ...
        pinButton.addEventListener('click', () => {
            if (detail.forced === 'active') handlers.onDeactivate(detail.itemKey);
            else handlers.onActivate(detail.itemKey);
        });
